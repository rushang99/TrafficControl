function ell205=check_whether_ambulance(audioe)
    ...
    %     t=max(w);
    %     end
    [r,t]=audioread(audioe);    
    sound(r,t);
    for p=1
        pause(4.5);
    end
    [r1,t1]=audioread('ambulance_template.wav');
    sound(r1,t1);
    for p=1
        pause(1.5);
    end
    if abs(project_conv('ambulance_template.wav',audioe)-1.4e+03)<0.2e+03
        %condition 1 that identifies a peak in the convolved signal of the
        %(in time domain) and if found within tolerance range
    ...
    %********************************************************************************************************************************************
        ...
        hold on
        [f2,Y2,newlength2]=project205(audioe);
